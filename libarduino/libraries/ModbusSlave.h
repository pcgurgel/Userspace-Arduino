...
const unsigned char fsupported[] = { FC_READ_COILS, FC_READ_INPUTS, FC_READ_REGS, FC_READ_INPUTREGS, FC_WRITE_COIL, FC_WRITE_REG, FC_WRITE_REGS };
 ...
 */
void ModbusSlave::build_read_packet(unsigned char function, unsigned char bytes, unsigned char *packet) 
{
        packet[SLAVE] = slave;
...
/*********************************************************************
 * 
 * 	validate_request(request_data_array, request_length)
 * 
...
int ModbusSlave::validate_request(unsigned char *data, unsigned char length) 
{
		...
		if (FC_WRITE_COIL == data[FUNC]) {
                /* For function write single coil, this is the target coil.*/
                coil_num = ((int) data[START_H] << 8) + (int) data[START_L];
                if (coil_num >= coils_size)
                        return EXC_ADDR_RANGE;
                return 0;
        }     
		...
		if (FC_READ_COILS == data[FUNC]) {
				coil_num = ((int) data[START_H] << 8) + (int) data[START_L];
				coil_num += ((int) data[REGS_H] << 8) + (int) data[REGS_L];    //plus quant coils
				if (coil_num > coils_size)
                        return EXC_ADDR_RANGE;
                return 0;
        }   
		
		if (FC_READ_INPUTS == data[FUNC]) {
				inputs_num = ((int) data[START_H] << 8) + (int) data[START_L];
				inputs_num += ((int) data[REGS_H] << 8) + (int) data[REGS_L];    //plus quant inputs
				if (inputs_num > inputs_size)
                        return EXC_ADDR_RANGE;
                return 0;
        }  
        ...
        if (FC_READ_REGS == data[FUNC] || FC_READ_INPUTREGS == data[FUNC])
		...
		if (FC_READ_REGS == data[FUNC] || FC_WRITE_REGS == data[FUNC]) {
		...
		if (FC_READ_INPUTREGS == data[FUNC]) {
			/* check registers range, start address is 0 */
			start_addr = ((int) data[START_H] << 8) + (int) data[START_L];
			if ((start_addr + regs_num) > inputregs_size)
				return EXC_ADDR_RANGE;
...
/************************************************************************
 * 
 * 	read_registers(first_register, number_of_registers, registers_array, function code)
 * 
 * 
 * reads the slave's holdings and input registers and sends them to the Modbus master
 * 
 *************************************************************************/
int ModbusSlave::read_registers( unsigned int start_addr, unsigned char reg_count, int *regs, unsigned char function) 
{
        int packet_size = 3;
        int status;
        unsigned int i;
        ...
        build_read_packet(function, reg_count * 2, packet);
...
/************************************************************************
 * 
 * write_single_coil(write_addr, data_array, coils_array)
 * 
 * Write a single boolean value. 
 * 
 *************************************************************************/
int ModbusSlave::write_single_coil(unsigned int write_addr, unsigned char *query, bool *coils) 
{
        unsigned char function = FC_WRITE_COIL; /* Function: Write Single Coil */
        int status = 0;
        unsigned int reg_val;
        unsigned char packet[RESPONSE_SIZE + CHECKSUM_SIZE];

        if (query[REGS_H] == 0x00 && query[REGS_L] == 0x00) {
			coils[write_addr] = false;
		} else {
			if (query[REGS_H] == 0xFF && query[REGS_L] == 0x00) {
				coils[write_addr] = true;
			} else {
				return -1;
			}
		}
...
/************************************************************************
 * 
 * read_booleans(first_boolean, number_of_booleans, array, function code)
 * 
 * Read coils and input status (boolean values) 
 * 
 *************************************************************************/
int ModbusSlave::read_booleans(unsigned int start_addr, unsigned char count, bool *vector, unsigned char function) 
{
		int packet_size = 3;
        int status;
        unsigned int i;
        unsigned char packet[MAX_MESSAGE_LENGTH];
		unsigned char byte;
		unsigned int pointer = start_addr;

		unsigned char bytes = count/8;
		if (count % 8 != 0) bytes++;
		
        build_read_packet(function, bytes, packet);

		for (int i = 0; i < bytes; i++) {
			packet[packet_size] = 0;
			for (int k = 0;  k < 8; k++) {
				byte = (unsigned char) vector[pointer];
				packet[packet_size] = packet[packet_size] | (byte << k);
				pointer++;
				if (pointer-start_addr >= count) break;
			}
			packet_size++;
		}
...
*/
void ModbusSlave::setup_regs(int* vector, unsigned int size) {
	regs = vector;
	regs_size = size;
}
...
*/
void ModbusSlave::setup_inputregs(int* vector, unsigned int size) {
	inputregs = vector;
	inputregs_size = size;
}
...
*/
void ModbusSlave::setup_coils(bool* vector, unsigned int size) {
	coils = vector;
	coils_size = size;
}
...
*/
void ModbusSlave::setup_inputs(bool* vector, unsigned int size) {
	inputs = vector;
	inputs_size = size;
}
